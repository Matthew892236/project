...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!shouldTrigger) return new Response(JSON.stringify({ message: "No escalation required" }), { status: 200 });

    const currentIndex = record.current_approach_index || 0;
    const list = record.approached_spares || [];

    if (currentIndex >= list.length) {
      await supabase.from('availability').update({ status: 'Not Available', approach_initiated_at: null }).match({ player_id: record.player_id, concert_id: record.concert_id });
      return new Response(JSON.stringify({ message: "Shortlist fully exhausted" }), { status: 200 });
    }

    const targetSpare = list[currentIndex];
    const { data: playerProfile } = await supabase.from('players').select('email').eq('id', targetSpare.id).single();
    const targetConcertId = record?.concert_id || record?.concertId;
    const { data: concertDetails } = await supabase.from('concerts').select('*').eq('id', targetConcertId).maybeSingle();
    ...
    if (foundBandId) {
      const { data: bandData } = await supabase.from('bands').select('*').eq('id', foundBandId).single();
      if (bandData) {
        ...
        if (bandData.manager_id) {
          ...
          } catch (e) { console.error("Manager lookup failed"); }
        }
      }
    }

    if (!playerProfile?.email) {
      await supabase.from('availability').update({ current_approach_index: currentIndex + 1, approach_initiated_at: new Date().toISOString() }).match({ player_id: record.player_id, concert_id: record.concert_id });
    ...
    const emailHtml = `
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; background-color: #f8fafc; padding: 24px; border-radius: 8px; border: 1px solid #e2e8f0;">
        ...
        <p style="font-size: 13px; color: #64748b; line-height: 1.5;">⏱️ <em>Note: If you do not respond, this invitation will automatically roll forward to the next player on our list.</em></p>
    ...
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({ from: `${bandName} Manager <Admin@brassbandwidth.com>`, reply_to: replyToEmail, to: playerProfile.email, subject: `Dep Request: ${bandName} - ${concertNameDisplay}`, html: emailHtml }),
    });
