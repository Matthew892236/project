...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const [concertsRes, playersRes, availRes, profileRes] = await Promise.all([
      ...
      // 🎯 FIXED: Added approached_spares, current_approach_index, and approach_initiated_at so the matrix UI can read them!
      supabase.from("availability").select("player_id, concert_id, status, spare_player_id, approached_spares, current_approach_index, approach_initiated_at").eq("user_id", uid),
