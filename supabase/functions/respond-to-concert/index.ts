      ...
      <div className="respond-card">
        ...
        <div className="respond-body">
          {state === 'loading' || state === 'submitting' ? (
            ...
            <div className="respond-done" style={{ textAlign: 'center', padding: '16px' }}>
              
              {/* 🌟 THE FIX: Extremely clear Thank You messaging that cannot be missed */}
              {directStatus === 'contact-manager' ? (
                <>
                  <div style={{ fontSize: '56px', marginBottom: '16px' }}>⚠️</div>
                  <h2 style={{ color: '#0f172a', fontSize: '24px', fontWeight: 'bold' }}>Action Blocked</h2>
                  <p style={{ fontSize: '16px', color: '#475569', marginTop: '12px', lineHeight: '1.5' }}>
                    This seat's availability has already been recorded! If you need to change your mind, please contact the band manager directly so they can update the system.
                  </p>
                </>
              ) : directStatus === 'accepted' ? (
                <>
                  <CheckCircle size={56} color="#16a34a" style={{ margin: '0 auto 16px auto' }} />
                  <h2 style={{ color: '#16a34a', fontSize: '24px', fontWeight: 'bold' }}>Thank You!</h2>
                  <p style={{ fontSize: '16px', color: '#334155', marginTop: '12px', lineHeight: '1.5' }}>
                    You have successfully <strong>accepted</strong> the gig! The band manager's system has been updated.
                  </p>
                </>
              ) : directStatus === 'declined' ? (
                <>
                  <XCircle size={56} color="#ea580c" style={{ margin: '0 auto 16px auto' }} />
                  <h2 style={{ color: '#ea580c', fontSize: '24px', fontWeight: 'bold' }}>Response Recorded</h2>
                  <p style={{ fontSize: '16px', color: '#334155', marginTop: '12px', lineHeight: '1.5' }}>
                    Thank you for letting us know quickly! You have successfully <strong>declined</strong> the gig. The manager's system has been updated.
                  </p>
                </>
              ) : directStatus === 'error' || directStatus === 'unknown' ? (
                <>
                  <XCircle size={56} color="#ef4444" style={{ margin: '0 auto 16px auto' }} />
                  <h2 style={{ color: '#ef4444', fontSize: '24px', fontWeight: 'bold' }}>Link Expired</h2>
                  <p style={{ fontSize: '16px', color: '#334155', marginTop: '12px', lineHeight: '1.5' }}>
                    We couldn't securely verify this link. Please contact the band manager directly to update your status.
                  </p>
                </>
              ) : (
                <>
                  <CheckCircle size={56} color="#3b82f6" style={{ margin: '0 auto 16px auto' }} />
                  <h2 style={{ color: '#3b82f6', fontSize: '24px', fontWeight: 'bold' }}>System Updated</h2>
                  <p style={{ fontSize: '16px', color: '#334155', marginTop: '12px', lineHeight: '1.5' }}>
                    Your status has been successfully logged in the database.
                  </p>
                </>
              )}
