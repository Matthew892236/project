...
function SortableRow({ player, concerts, allPlayers, globalSpares, activeDropdown, setActiveDropdown, getAvailability, onSetStatus, onAddPlayer, getAvailableSpares, renderDepRow, openCascadeCompose }: any) {
  ...
  // Ensure transform is undefined when not dragging to prevent breaking sticky positioning
  const rowStyle = { transform: transform ? CSS.Transform.toString(transform) : undefined, transition, opacity: isDragging ? 0.4 : 1 };
            ...
            {activeDropdown === cellId && anchorRect && (
              <PortalDropdown anchorRect={anchorRect} onClose={() => setActiveDropdown(null)}>
                <div style={{ maxHeight: '380px', overflowY: 'auto' }}>
                  ...
                  <div style={{ padding: '10px 12px', fontSize: '11px', fontWeight: 700, color: '#64748b', textTransform: 'uppercase', borderTop: '1px solid #e2e8f0', marginTop: '4px' }}>Online Network Deps ({globalSparesList.length})</div>
  ...
  return (
    <div style={{ padding: '32px', fontFamily: 'system-ui', maxWidth: '1400px', margin: '0 auto', boxSizing: 'border-box' }}>
      ...
      {concerts.length === 0 ? (
        ...
        <div style={{ width: '100%', overflowX: 'auto', position: 'relative', paddingBottom: '16px', backgroundColor: '#fff', borderRadius: '12px', border: '1px solid #e2e8f0', boxShadow: '0 1px 3px rgba(0,0,0,0.05)', maxWidth: '100vw' }}>
                            ...
                            {vacantDropdown === cellId && vacantAnchor && (
                              <PortalDropdown anchorRect={vacantAnchor} onClose={() => setVacantDropdown(null)}>
                                <div style={{ maxHeight: '350px', overflowY: 'auto' }}>
                                  ...
                                  <div style={{ padding: '10px 12px', fontSize: '11px', fontWeight: 700, color: '#64748b', textTransform: 'uppercase', borderTop: '1px solid #e2e8f0', marginTop: '4px' }}>Online Network Deps ({globalS.length})</div>
