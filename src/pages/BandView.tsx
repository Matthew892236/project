...
function statusColor(status: string) {
  ...
  if (status === 'Spares Contacted') return '#ffedd5'; 
  return '#f3f4f6'; 
}
...
function statusText(avail: Availability, players: Player[]) {
  ...
  if (avail.status === 'Spare Assigned') {
    ...
    return { label: spareName ? `Spare: ${spareName.split(' ')[0]}` : 'Spare Covered', color: '#1e40af' }; 
  }
  ...
  if (avail.status === 'Spares Contacted') {
    ...
    return { label: currentSpare ? `Asked: ${currentSpare.name.split(' ')[0]}` : 'Checking Spares...', color: '#9a3412' }; 
  }
